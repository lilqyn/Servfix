...
type BlogPost = NonNullable<StaticPageContent["posts"]>[number];
    ...
    <View style={styles.featuredCard}>
      {post.imageSignedUrl || post.image ? (
        ...
        <View style={styles.featuredImagePlaceholder}>
          <Ionicons name="newspaper-outline" size={48} color={palette.line} />
        </View>
      )}
...
export default function BlogScreen({ onBack }: BlogScreenProps) {
  const styles = useStyles();
  const { palette } = useTheme();
  ...
  const renderContent = () => {
    ...
    if (posts.length === 0) {
      return (
        <View style={styles.center}>
          <Ionicons name="document-text-outline" size={56} color={palette.line} />
